...
const Modal = ({ handleClose, type }) => {
  ...
  const {posts,selectedPost} = useStates();
  ...
  return (
    <Backdrop onClick={handleClose}>
      {type === "dropIn" && (
        ...
        >
          <div className="flex items-center justify-between border-b border-white/75 px-4 py-2.5">
            <h4 className="text-xl">Share Your Thoughts</h4>
      ...
      />}
          {<div className="w-full md:w-3/5 bg-white dark:bg-gray-800 rounded-r-lg">
            <Post post={selectedPost} modalPost />
    </div>}
        </motion.div>
      )}
    </Backdrop>
